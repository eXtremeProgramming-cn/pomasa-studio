...
三个页面：MAS 列表、新建表单、MAS 详情。

流转：列表到新建表单，提交后回列表显示生成中卡片，生成完成后进入详情。
...
- multi：运行选择器 = 单元列表，每个单元一项，显示键名与状态（已枚举未运行、运行中、已完成、失败）；提供"运行全部"（批量，BHV-03 并行）和"运行选中的单元"。若单元由运行期枚举，列表还包括"已枚举未运行"的提示，如已枚举 120 国，已运行 34。
