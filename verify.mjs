...
test('L2 lifecycle: create -> generating -> completed', async () => {
  ...
  assert.ok(agents.has('pomasa-gen-demo2'))
  const genAgent = agents.get('pomasa-gen-demo2')
  assert.equal(genAgent.calls[0][0], 'followup')
  assert.match(genAgent.calls[0][1].content, /SKILL.md/)
  assert.match(genAgent.calls[0][1].content, /demo2/)
...
test('L2 lifecycle: run.start spawns sessions, intervene/cancel route', async () => {
  ...
  const runAgent = agents.get('pomasa-run-demo-single')
  assert.ok(runAgent)
  assert.match(runAgent.calls[0][1].content, /00\.orchestrator\.md/)
  assert.match(runAgent.calls[0][1].content, /workspace/)

  const iv = await call(routes, '/pomasa/run.intervene', 'POST', { masId: 'demo', unit: null, message: '再搜一下背景' })
  assert.equal(iv.json.ok, true)
  assert.equal(runAgent.calls[1][1].content, '再搜一下背景')
